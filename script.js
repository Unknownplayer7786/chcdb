...
function forceResizeAll() {
    setTimeout(() => { document.querySelectorAll('.auto-expand').forEach(adjustHeight); }, 150);
}
...
async function handleLogin() {
    ...
    } else { alert("WRONG CLOUD PASSCODE"); status.classList.add('hidden'); }
}

async function updatePassword() {
    const newPass = document.getElementById('new-password').value;
    if(newPass.length < 4) return alert("Must be 4+ digits");
    await _supabase.from('clinic_settings').upsert({ id: 'global_config', prefix: state.prefix, counter: state.counter, passcode: newPass });
    state.masterPass = newPass; alert("Passcode Updated Globally");
}
...
async function saveClinicSettings() {
    ...
    state.prefix = p; state.counter = c; alert("Formatting Synced");
}
...
async function handleSearch() {
    ...
    document.getElementById('search-results').innerHTML = data.map(r => `
        <div class="p-6 bg-white border border-[#e2d8c3] rounded-3xl flex justify-between items-center group transition hover:border-[#7c9082] shadow-sm" onclick="editRecord('${r.id}')">
            <div class="flex items-center gap-5">
                ...
                <div><div class="text-[9px] font-black tracking-widest text-[#7c9082]">ID: ${r.case_number}</div><div class="font-bold text-xl">${r.full_name}</div></div>
            </div>
...
async function saveRecord() {
    const fields = ['caseNumber', 'name', 'middleName', 'surname', 'address', 'city', 'mobile', 'email', 'age', 'dob', 'bloodGroup', 'pinCode'];
    const data = { id: state.editingId || Date.now().toString(), photo: state.currentPhoto, symptoms: [...state.symptoms] };
    fields.forEach(f => {
        const el = document.getElementById(`field-${f}`);
        if(el) data[f] = el.value;
    });
    if(!data.name || !data.caseNumber) return alert("Missing Required Name or ID");
...
async function editRecord(id) {
    ...
    ['caseNumber', 'name', 'middleName', 'surname', 'address', 'city', 'mobile', 'email', 'age', 'dob', 'bloodGroup', 'pinCode'].forEach(f => {
    ...
    renderSymptoms(); switchTab('entry');
}
...
function renderSymptoms() {
    document.getElementById('symptom-body').innerHTML = state.symptoms.map(s => `
        <tr>
            ...
            <td class="px-6 py-2 align-top border-l border-[#f2ede4]"><textarea oninput="adjustHeight(this); updSym(${s.id},'remedy1',this.value)" class="auto-expand" placeholder="Remedy 1...">${s.remedy1 || ""}</textarea></td>
            <td class="px-6 py-2 align-top border-l border-[#f2ede4]"><textarea oninput="adjustHeight(this); updSym(${s.id},'remedy2',this.value)" class="auto-expand" placeholder="Remedy 2...">${s.remedy2 || ""}</textarea></td>
            <td class="px-2 py-2 align-top text-center no-print"><button onclick="remSymRow(${s.id})" class="text-[#dcd7c9] hover:text-red-400 font-bold text-xl">×</button></td>
        </tr>`).join('');
    forceResizeAll();
}
...
async function deleteRecord(id) { if (!id || !confirm("Delete case permanently?")) return; await _supabase.from('patients').delete().eq('id', id); resetForm(); switchTab('dashboard'); }
function handleLogout() { location.reload(); }
async function exportToPDF() { const canvas = await html2canvas(document.getElementById('printable-record'), { backgroundColor: '#ffffff', scale: 2 }); const pdf = new jspdf.jsPDF('p', 'mm', 'a4'); pdf.addImage(canvas.toDataURL('image/png'), 'PNG', 0, 0, 210, (canvas.height * 210) / canvas.width); pdf.save("Patient_Case.pdf"); }
